...
describe('<CityAdder/>', () => {
    const wrapper = mount(
        <CityAdder onTagClick={tagClick} cityEntities={cityEntities} />
    );
    ...
    it('renders tags from the cityEntities list', () => {
        expect(wrapper.find('div.city-adder__tag').length).toBe(3);
    });
    it('simulates a button click', async () => {
        const tag = wrapper.find('.city-adder__tag').first();
        tag.simulate('click');
        await wait(600);
        expect(tagClick).toBeCalled();
    });
});
